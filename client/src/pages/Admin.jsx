    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <form onSubmit={addPrediction} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <input
                value={form.option0}
                onChange={e => setForm(f => ({ ...f, option0: e.target.value }))}
                placeholder="ตัวเลือก 1"
                className="border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-green-400"
                required
              />
              <input
                value={form.option1}
        ...
        <div className="space-y-3">
          <h2 className="font-bold text-gray-800">📋 คำทายทั้งหมด</h2>
          {predictions.map(p => (
            <div key={p.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                {p.options.map((opt, idx) => (
                  <div key={idx} className={`text-xs px-3 py-1.5 rounded-xl text-center font-medium ${
                    p.resolved && p.correct_option === idx
              ...
              {!p.resolved ? (
                <div className="flex gap-2">
                  <button
                    onClick={() => resolve(p.id, 0)}
                    className="flex-1 text-xs py-1.5 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium"
                  >
                    เฉลย: {p.options[0]}
                  </button>
                  <button
                    onClick={() => resolve(p.id, 1)}
                    className="flex-1 text-xs py-1.5 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium"
                  >
                    เฉลย: {p.options[1]}
                  </button>
                  <button
                    ...
                    className="text-xs py-1.5 px-3 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 font-medium"
                  >
                    ลบ
                  </button>
                </div>
                ...
                <p className="text-xs text-green-600 font-medium text-center">
                  ✅ เฉลยแล้ว: {p.options[p.correct_option]}
                </p>
              )}
            </div>
          ))}
